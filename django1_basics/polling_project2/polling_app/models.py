...
# The name of each Field instance (e.g. question_text or pub_date) is the field’s name,
 # in machine-friendly format. You’ll use this value in your Python code, and your database will use it as the column name.
#
...
# If this field isn’t provided, Django will use the machine-readable name. In this example, we’ve only defined a human-readable name for Question.pub_date. For all other fields in this model, the field’s machine-readable name will suffice as its human-readable name.
#
...
#
# Finally, note a relationship is defined, using ForeignKey. That tells Django each Choice
 # is related to a single Question. Django supports all the common database relationships: many-to-one, many-to-many
 # , and one-to-one.
